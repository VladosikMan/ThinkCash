...
public class GraphikTable2 extends ImageView {
    private Paint mPaint;
    private String[] week={"Пн","Вт","Ср","Чт","Пт","Сб","Вс"};
    private String[] year={"Я","Ф","М","А","М","И","И","А","С","О","Н","Д"};
    private String[] month;
    public GraphikTable2(Context context, AttributeSet attrs) { super(context, attrs); }
    public GraphikTable2(Context context,  AttributeSet attrs, int defStyleAttr) { super(context, attrs, defStyleAttr); }
    public GraphikTable2(Context context,  AttributeSet attrs, int defStyleAttr, int defStyleRes) { super(context, attrs, defStyleAttr, defStyleRes); }
    @Override
    protected void onDraw(Canvas canvas) {
        ...
        po[0].x=80;
        po[0].y=350;
        double omec =((double)StorGraph.maxR)/300;
        for(i=1;i<=StorGraph.masikR.length;i++){
            po[i]=new Point();
            ...
            switch (DataTime.typeInterval){
                case "d":{
                    ...
                    po[i].x=80+i*75;
                    //вертикальная точка
                    po[i].y= (float) (350-(StorGraph.masikR[i-1]/omec));
                    break;
                }
                case "w":{
                    // горизонтальная точка
