    ...
    protected void onDraw(Canvas canvas)
    {
       ...
       cy = h / 2;
           ...
           if (StorDrawCat1.summ == 0) {
               ...
               mPaint.setColor(Color.BLACK);
               canvas.drawText(Integer.toString(StorDrawCat1.summ), cx, cy, mPaint);
           } else {
               double procef = 0;
               mPaint.setColor(Color.WHITE);
               mPaint.setStrokeWidth(2);
               ...
               canvas.drawLine(x + cx, y + cy, x1 + cx, y1 + cy, mPaint);
               int i;
               for (i = 0; i < StorDrawCat1.dog.size(); i++) {
                   byte type = 0;
                   ...
                   if (type == StorDrawCat1.type) {
                       if ((StorDrawCat1.dog.get(i).money != 0) && (StorDrawCat1.dog.get(i).percent > 2)) {
                           mPaint.setARGB(127, StorDrawCat1.dog.get(i).r, StorDrawCat1.dog.get(i).g, StorDrawCat1.dog.get(i).b);
                           fi = (float) (fi + 0.2);
                           procef = procef + StorDrawCat1.dog.get(i).percent;
                           while (fi < procef) {
                               fix = (float) ((3.6 * fi) * Math.PI) / 180;
                               x = (float) Math.floor(rad * Math.cos(fix));
                               y = (float) Math.floor(rad * Math.sin(fix));
               ...
               canvas.drawText(Integer.toString(StorDrawCat1.summ), cx-20, cy, mPaint);
        }
